...
void receiveEvent(int bytes) {
  hapticLimit = Wire.read();//Receive value from master board
  Serial.print("Reveived from master: "+ hapticLimit);
}
...
void loop() {
  if (comm->isOpen()){
    ...
    comm->output(encode(fingerPos, getJoyX(), getJoyY(), joyButton, triggerButton, aButton, bButton, grabButton, pinchButton, calibButton, menuButton));
      ...
      // String str[5] = {"","A662B0C790D600E272","A662B100C790D600E272","A662B700C790D600E272","A662B800C790D600E272"};
      // if (str[loops%5].length()>0){
      //   str[loops%5].toCharArray(received,str[loops%5].length());
      //   Serial.println("before received: "+ String(received));
      // }
      // Serial.println("strFromMaster: "+ strFromMaster);
      //  strFromMaster.toCharArray(received, strFromMaster.length());
      ...
      if (comm->readData(received)){
        ...
        int hapticLimits[5] = {0,hapticLimit,hapticLimit,hapticLimit,hapticLimit};
         Serial.println("limits test: "+ String(hapticLimits[1]));
        writeServoHaptics(hapticLimits); 
      }
    #endif
    delay(LOOP_TIME);
